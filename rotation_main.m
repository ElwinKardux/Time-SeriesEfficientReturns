...
eff_PC = PCA.eff_PC;
rot_PC_eff = PCA.rot_PC_eff;
rot_PC_MA = PCA.rot_PC_MA;
rot_PC_PM = PCA.rot_PC_PM;

%important
rot_old = rot.rot_old;
rot_ar1_eff = rot.rot_ar1_eff;
rot_ar12_eff = rot.rot_ar12_eff;
rot_cross_eff = rot.rot_cross_eff;
...
model_PC_eff= fitlm(ff5effX, rot_PC_eff+0.25);
...
%% 
cumproducts = zeros(8,1);
% 

%cumproducts(1) =  cumplot(rot_old, 'Orignal Industry returns');
% cumproducts(2) =  cumplot(rot_ar1_eff, 'Inter- Industry Returns') ;
% cumproducts(3) =  cumplot(rot_ar12_eff, 'Prior- Year Efficient Returns');
% cumproducts(4) =  cumplot(rot_cross_eff, 'Cross- Industry Efficient Returns');
% cumproducts(5) =  cumplot(rot_old_MA, 'Original Moving Average');
% cumproducts(6) =  cumplot(rot_ar1_MA ,'Inter- Industry Returns');
% cumproducts(7) =  cumplot(rot_ar12_MA, 'Prior- Year Efficient Returns') ;
% cumproducts(8) =  cumplot(rot_cross_MA, 'Cross- Industry Efficient Returns'); 

%cumplot2(rot_old, rot_old_MA,original,  'Original Returns') ;
%cumplot2(rot_old, rot_old_MA, ff5oud(end-422:end,1),  'Original Returns');
%cumplot2(rot_ar1_eff, rot_ar1_MA, eff_inter,  'Inter- Industry Efficient Returns');
%cumplot2(rot_ar12_eff, rot_ar12_MA, eff_prior,  'Prior- year Efficient Returns');
%cumplot2(rot_cross_eff, rot_cross_MA, eff_cross,  'Cross-Industry Efficient Returns');

%cumplot2(rot_old + 0.55, rot_old_MA,original,  'Original Returns') ;
%cumplot2(rot_old +0.4, rot_old_MA, original,  'Original Returns');
%cumplot2(rot_ar1_eff+0.4, rot_ar1_MA, eff_inter,  'Inter- Industry Efficient Returns');
%cumplot2(rot_ar12_eff+0.3, rot_ar12_MA, eff_prior,  'Prior- year Efficient Returns');
%cumplot2(rot_cross_eff+0.5, rot_cross_MA, eff_cross,  'Cross-Industry Efficient Returns');

%cumplot3(rot_old+0.25, rot_ar1_eff+0.25, rot_ar12_eff+0.25, rot_cross_eff +0.25, '(B) Adaptive Lasso Rotation Portfolio')
%cumplot3(rot_old_MA, rot_ar12_MA, rot_ar1_MA, rot_cross_MA , '(C) Cross-Sectional Momentum Rotation Portfolio')
%cumplot3(rot_old_PM, rot_ar1_PM, rot_ar12_PM, rot_cross_PM, '(A) Prevailing Mean')
...
function c = cumplot(returns, titles)
...
annRetStd = strcat("[", num2str(round(std(returns),2)),"]");
...
text(time(5),2.85,txt1,'FontSize',13)
text(time(5),2.7,txt2,'FontSize',13)
%ylim manual
ylim([-0.5, 3]);
ax.XTick = time(1:60:end);
datetick('x', 'yyyy', 'keepticks')
xlabel('year');
title(titles);
ax.FontSize = 12;
recessionplot;
set(gcf,'units','points','position',[10,10,700,400])
end
...
%annRetStd = strcat("[", num2str(round(std(returns),2)),"]");
%MaxDD = num2str(round(maxdrawdown(cumprod(returns/100 + 1.0))*100,2));
...
function cumplot3(returns, returns2, returns3, returns4, titles)
returns11 = returns/std(returns);
returns22 = returns2/std(returns2);
returns33 = returns3/std(returns3);
...
recessionplot;
set(gcf,'units','points','position',[10,10,400,250])
legend({'Original returns','Inter-Industry', 'Prior-year', 'Cross-Industry'},'Location','northwest','FontSize',10)
end
